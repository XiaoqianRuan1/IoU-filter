...
class CityscapesDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, split, use_difficult=False, transforms=None):
        ...
        self.rainy = "rain_val" in split
        
        if split == 'foggy_train':
            split = 'train'
            self._root_img_path = os.path.join(self.root,'leftImg8bit_foggy', split)
            self._imgpath = os.path.join(self._root_img_path, '%s', '%s_leftImg8bit_foggy_beta_0.005.png')
            ...
            split = 'val'
            self._root_img_path = os.path.join(self.root, 'leftImg8bit_rain', split)
            self._imgpath = os.path.join(self._root_img_path, '%s', '%s_leftImg8bit_rain_alpha_0.03_beta_0.015_dropsize_0.002_pattern_12.png')
        else:
            self._root_img_path = os.path.join(self.root, 'leftImg8bit', split)
            self._imgpath = os.path.join(self._root_img_path, '%s', '%s_leftImg8bit.png')

        self._annopath = os.path.join(self.root, 'gtFine', split, '%s', '%s_gtFine_polygons.json')
        ...
        for city in os.listdir(os.path.join(self._root_img_path)):
            ...
            if self.foggy:
                ...
                print("!!!!!!!!!!!!!!!!!!!")
                city_ids = list(filter(lambda x: x.split('_')[-1] == '12.png',city_ids))
                end_prefix = -10
            else:
                end_prefix = -1
